...
public:
	...
	void SetBusEffect(struct FName BusName, struct FName EffectSetting, int32_t EffectSlot);
	void ResetSlapbackObjects();
	void SetSoundEnvironment(class AActor* Actor, uint8_t EnvironmentType);
	class UAkEnvironments* GetEnvironments();
	void NotifyWhenInitialized(struct FScriptDelegate Callback);
	void ProfileStop();
	void ProfileStart();
	void PrintData();
	void SetDynamicRange(uint8_t Type);
	void SetOutputType(uint8_t Type);
	void SetListenerSpatialization(class ULocalPlayer* Player, uint8_t Value);
...
public:
	...
	void OnNewTrack(int32_t NewBPM);
...
public:
	...
	void PrintDebugInfo(class UDebugDrawer* Drawer);
	float InterpGearRatio(int32_t Gear, struct FAkRevSimUpdateParams& SimUpdate);
...
public:
	...
	void EventMarker(class UAkSoundSource* Source, int32_t PlayingID, struct FString Marker);
...
public:
	...
	uint8_t                                            UnknownData01[0x108];                          // 0x0090 (0x0108) MISSED OFFSET
public:
	static UClass* StaticClass()
	{
		static UClass* uClassPointer = nullptr;
